...
public class EightPuzzle implements SearchProblem<List<Integer>> {
  ...
  public EightPuzzle(List<Integer> startingValues) {
	  ...
	  values=new ArrayList<Integer>(startingValues);
  ...
  private List<Integer> swap(List<Integer> list, int a, int b){
  ...
  public List<List<Integer>> getSuccessors(List<Integer> currentState) {
	  List<Integer> cur = new ArrayList<>(currentState);	  
      successors = new ArrayList<List<Integer>>();
      int index = currentState.indexOf(0);
      if(index>=3){			  
		  successors.add(swap(cur,index,index-3));
	  }
	  if(index<=5){
		  successors.add(swap(cur,index,index+3));	  
	  }
	  if(index!=0&&index!=3&&index!=6){
		  successors.add(swap(cur,index,index-1));
	  }
  ...
  public boolean isGoal(List<Integer> state) {
    // TODO
    return state.equals(Arrays.asList(new Integer[] { 1, 2, 3,
        4, 5, 6, 7, 8, 0 }));
  }
  public static void printpuzzle(List<Integer> list){
  ...
  public static void main(String[] args) {
	  Integer[] arr = new Integer[] { 1, 2, 3, 4, 0, 6, 7, 5, 8 };
	  
      EightPuzzle e = new EightPuzzle(Arrays.asList(arr));
